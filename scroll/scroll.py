...
class Scroll(commands.Cog):
	...
	async def ActivePing(self, ctx):
		...
		eventlist = BeautifulSoup(req.text, "xml").find_all("EVENT")
		timeslist = BeautifulSoup(req.text, "xml").find_all("TIMESTAMP")
		regionTextlist = BeautifulSoup(req.text, "xml").find_all("TEXT")
		regionlist = []
	...
	async def QueuePing(self, ctx):
		...
		if len(eventlist) == 100:	
			...
			while caughtUp == False:
				...
				list1 = BeautifulSoup(req.text, "xml").find_all("EVENT")
				list2 = BeautifulSoup(req.text, "xml").find_all("TIMESTAMP")
				list3 = BeautifulSoup(req.text, "xml").find_all("TEXT")
	...
	async def tgqueue(self, ctx):
		...
		soup = BeautifulSoup(queuereq.text, "xml")
