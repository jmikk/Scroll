...
class ApprovalView(discord.ui.View):
    def __init__(self, recDict, timeout=600):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Find remaining users who need to approve
        remaining = user_ids - self.approved_users
...
class Scroll(commands.Cog):
    ...
    async def QueuePing(self, ctx):
        ...
        if len(eventlist) == 100:
            ...
            while caughtUp == False:
                ...
                list1 = BeautifulSoup(req.text, "html.parser").find_all("EVENT")
                list2 = BeautifulSoup(req.text, "html.parser").find_all("TIMESTAMP")
                list3 = BeautifulSoup(req.text, "html.parser").find_all("TEXT")
                ...
                if (
                    (len(list1) < 100)
                    or ((time.time() - int(list2[-1].string)) > 172000)
        ...
        for key in queueDict:
            ...
            for a in queueDict[key][0]:
                ...
                try:
                    if (
                        time.time() - queueDict[key][2][b]
                    ) < 172000:  # and (queueDict[key][0][a] not in ctelist):
    ...
    async def tgqueue(self, ctx):
        ...
        soup = BeautifulSoup(queuereq.text, "html.parser")
