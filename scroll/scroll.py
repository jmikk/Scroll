...
global numeralmatch
numeralmatch = re.compile(
    r"(?i)[-_ ](?=[MDCLXVI]+\b)(M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3}))$"
)
global delayTime
delayTime = 60
...
class Scroll(commands.Cog):
        ...
        for key in recDict:
            if len(queueDict[key][0]) >= 8:
                ...
                item = discord.ui.Button(
                    ...
                    url="https://www.nationstates.net/page=compose_telegram?tgto={recipients}&message={message}&generated_by=scroll_discord_bot__by_devi__instance_run_by_{user}".format(
                        recipients=str(natList)[1:-1].replace("'", "").replace(" ", ""),
                        message=recDict[key][0][2].replace("%", "%25"),
                ...
                await ctx.send(
                    "Not enough nations could be found, please wait a moment."
                )
    ...
    async def ActivePing(self, ctx):
        ...
        req = requests.get(
            f"https://www.nationstates.net/cgi-bin/api.cgi?q=happenings;filter=founding;sinceid={lastID}",
            headers=headers,
        )
        ...
        # Extract timestamps and region text
        ...
        for text in regionTextlist:
            ...
            if len(split_text) > 1:
                regionlist.append(split_text[1])
            else:
                regionlist.append("unknown_region")
        ...
        if len(eventlist) > 0:
            ...
            for count, a in enumerate(range(len(eventlist))):
                b = eventlist[a].find("TEXT").text.split("@@")[1]
                # if there's a roman numeral at the end, or an arabic number period, we run a check for if the nation can accept recruitment TGs
                if (
                    not any(char.isdigit() for char in str(b))
                    and not re.search(numeralmatch, b)
                    and regionlist[count] not in regionWhiteList
                ):
                    await asyncio.sleep(0.7)
                    req2 = requests.get(
                        f"https://www.nationstates.net/cgi-bin/api.cgi?nation={b}&q=tgcanrecruit",
                        headers=headers,
                    )
                    if "1" in req2.text:
                        activeQueue.append(
        ...
        # Add all the usable stuff to both the default backlog queue, and the backlog for each registered region
        ...
        for key in recDict:
            ...
            if len(queueDict[key][0]) >= numRecruiters * 8:
                tempQueue = [
                    queueDict[key][0][-(numRecruiters * 8) :],
                    queueDict[key][1][-(numRecruiters * 8) :],
                    queueDict[key][2][-(numRecruiters * 8) :],
                ]
                queueDict[key] = [
                    queueDict[key][0][: -(numRecruiters * 8)],
                    queueDict[key][1][: -(numRecruiters * 8)],
                ...
                print(
                    f"[{datetime.datetime.fromtimestamp(time.time()).strftime('%H:%M:%S')}] {str(sendList)}"
                )
                ...
                for a in sendList:
                    item = discord.ui.Button(
                        ...
                        label=str(a[0][1]),
                        url="https://www.nationstates.net/page=compose_telegram?tgto={recipients}&message={message}&generated_by=scroll_discord_bot__by_devi__instance_run_by_{user}".format(
                            recipients=str(a[1])[1:-1]
                            .replace("'", "")
                            .replace(" ", ""),
                            message=a[0][2].replace("%", "%25"),
                            user=headers["User-Agent"]
                            .split(":")[-1][1:-1]
                            .replace(" ", "_"),
                        ),
                ...
                if len(queueDict[key][0]) >= numRecruiters:
                    ...
                    for a in sendList:
                        item = discord.ui.Button(
                            style=discord.ButtonStyle.gray,
                            label=str(a[0][1]),
                            url="https://www.nationstates.net/page=compose_telegram?tgto={recipients}&message={message}&generated_by=scroll_discord_bot__by_devi____instance_run_by_{user}".format(
                                recipients=str(a[1])[1:-1]
                                .replace("'", "")
                                .replace(" ", ""),
                                message=a[0][2].replace("%", "%25"),
                    ...
                    await ctx.send(
                        f"not enough new nations have spawned to distribute to `{key}`, please wait a little longer."
                    )
    ...
    async def start(self, ctx, templatenumber: str):
        ...
        if headers == False:
            ...
            headers = {
                "User-Agent": f"Scroll manual recruitment cog for Discord's RedBot (developer:devi / valkynora@gmail.com; current instance run by: {agent})"
            }
        ...
        if inSession == False:
            ...
            if (time.time() - stopTime) < (delayTime + 10):
                ...
                return
            recDict = {
                tempRegion: [[str(author.id), str(author.display_name), tempString]]
            }
            channel = ctx.channel.id
            souplist2 = []
            await ctx.send(
                f"{author.mention} is now recruiting using template `{tempString}`."
            )
            inSession = True
            ...
            if not (ctx.channel.id == channel):
                ...
                if not (str(author.id) in lbDict):
                    lbDict[str(author.id)] = [0, str(author.display_name)]
                if not (str(author.id) in lbRegDict[tempRegion]):
                    lbRegDict[tempRegion][str(author.id)] = [
                        0,
                        str(author.display_name),
                    ]
                if tempRegion in recDict:
                    recDict[tempRegion].append(
                        [str(author.id), str(author.display_name), tempString]
                    )
                    await ctx.send(
                        f"{author.mention} is now recruiting using template `{tempString}`."
                    )
    ...
    async def forcestop(self, ctx):
        ...
        if not (headers):
            ...
            if agentPath[1] == False:
                ...
                with open(agentPath[0], "r") as f:
                    headers = {
                        "User-Agent": f"Scroll manual recruitment cog for Discord's RedBot (developer:devi / valkynora@gmail.com; current instance run by: {str(f.readline())})"
                    }
    ...
    async def PassiveLoop(self, ctx):
        ...
        while isRunning == True:
            ...
            if not (inSession):
                ...
                print(
                    f"[{datetime.datetime.fromtimestamp(time.time()).strftime('%H:%M:%S')}] ping"
                )
    ...
    async def QueuePing(self, ctx):
        ...
        if not lastID:
            ...
            if lastPath[1] == False:
                ...
                root = ET.fromstring(req.text)
                eventlist = root.findall("HAPPENINGS/EVENT")
                timeslist = [event.find("TIMESTAMP").text for event in eventlist]
                regionTextlist = [event.find("TEXT").text for event in eventlist]
                ...
                ctelist = []
                list1 = []
                # api batches come ordered new>old; we want old>new for this
                eventlist.reverse()
                timeslist.reverse()
                for count, a in enumerate(range(len(eventlist))):
                    ...
                    if (
                        "founded"
                        in eventlist[a].find("TEXT").text.split("@@")[2].split("%%")[0]
                    ):
                        # check if the happening isn't too old (2d cutoff)
                        if not (time.time() - int(timeslist[a])) > 172000:
                            # basic number filter; do a tgcanrecruit check if there's none
                            if (
                                not any(char.isdigit() for char in b)
                                and not re.search(numeralmatch, b)
                                and not regionlist[count] in regionWhiteList
                            ):
                                await asyncio.sleep(0.7)
                                req2 = requests.get(
                                    f"https://www.nationstates.net/cgi-bin/api.cgi?nation={b}&q=tgcanrecruit",
        ...
        req = requests.get(
            f"https://www.nationstates.net/cgi-bin/api.cgi?q=happenings;filter=founding+cte;limit=100;sinceid={lastID}",
            headers=headers,
        )
        root = ET.fromstring(req.text)
        eventlist = root.findall("HAPPENINGS/EVENT")
        ...
        if len(eventlist) == 100:
            ...
            while not caughtUp:
                await asyncio.sleep(0.7)
                req = requests.get(
                    f"https://www.nationstates.net/cgi-bin/api.cgi?q=happenings;filter=founding+cte;limit=100;sinceid={lastID};beforeid={lastID2}",
                    headers=headers,
                )
                root = ET.fromstring(req.text)
                list1 = root.findall("HAPPENINGS/EVENT")
                list2 = [event.find("TIMESTAMP").text for event in list1]
                list3 = [event.find("TEXT").text for event in list1]
                ...
                if (
                    len(list1) < 100
                    or (time.time() - int(list2[-1])) > 172000
                    or len(timeslist) >= 3500
                ):
                    caughtUp = True
                print(caughtUp)
        ...
        for count, a in enumerate(range(len(eventlist))):
            ...
            if (
                ...
                if (
                    ...
                    if "1" in req2.text:
                        print(b)
                        list1.append([b, eventlist[a].get("id"), int(timeslist[a])])
            else:
                ctelist.append(b)
        ...
        for key in queueDict:
            queueDict[key] = [
                queueDict[key][0][-1500:],
                queueDict[key][1][-1500:],
                queueDict[key][2][-1500:],
            ]
            tempa = []
            tempb = []
            tempc = []
            ...
            for a in queueDict[key][0]:
                ...
                try:
                    if (time.time() - queueDict[key][2][b]) < 172000:
            ...
            if queueDict == False:
                if queuePath[1] == False:
                    ...
                    try:
                        ...
                        await ctx.send(
                            "Queue data corrupted, repopulating with a blank queue"
                        )
                        queueDict = {"DEFAULT": [[], [], []]}
                        with open(queuePath[0], "w") as f:
                            f.write(str(queueDict))
    ...
    async def add(self, ctx, *param: str):
        ...
        if len(param) < 2:
            ...
            if "region=" in param[1]:
                ...
                await ctx.send(
                    f"{author.mention}: An invalid region URL has been provided."
                )
    ...
    async def leaderboards(self, ctx):
        ...
        if lbDict == {}:
            if lbPath[1] == False:
                await ctx.send(
                    "No leaderboard data has been found; please start recruiting to populate the leaderboards."
                )
                return
            else:
                with open(lbPath[0], "r") as f:
        ...
        for key in {
            k: v
            for k, v in sorted(lbDict.items(), key=lambda item: item[1], reverse=True)
        }:
            sendstring += (
                f"{str(num)}. `{str(lbDict[key][1])}`: {str(lbDict[key][0])}\n"
            )
            num += 1
        await ctx.send(sendstring)
    ...
    async def regionboards(self, ctx, *, region: str):
        ...
        if lbRegDict == {}:
            if lbRegPath[1] == False:
                await ctx.send(
                    "No regional leaderboard data has been found, please recruit to populate the leaderboards."
                )
                return
            else:
                with open(lbRegPath[0], "r") as f:
        ...
        if region in lbRegDict:
            ...
            for key in {
                k: v
                for k, v in sorted(
                    lbRegDict[region].items(), key=lambda item: item[1], reverse=True
                )
            }:
            ...
            await ctx.send(
                "No region with that name could be found in leaderboard data."
            )
    ...
    async def tgqueue(self, ctx):
        ...
        queuereq = requests.get(
            "https://www.nationstates.net/cgi-bin/api.cgi?q=tgqueue", headers=headers
        )

        # Parsing XML with ElementTree
        root = ET.fromstring(queuereq.text)
        manual = root.findtext("TGQUEUE/MANUAL")
        mass = root.findtext("TGQUEUE/MASS")
        api = root.findtext("TGQUEUE/API")
    ...
    async def delay(self, ctx, delay: int):
        ...
        try:
            ...
            if 50 <= delay <= 250:
                ...
                await ctx.send(
                    "The provided value is outside of the given range. Please use a whole number between 50 and 250 seconds."
                )
        except:
            await ctx.send(
                "The value given is not a valid integer. Please input a whole number between 50 and 250"
            )
    ...
    async def regionWhiteList(self, ctx, *regions):
        ...
        await ctx.send(
            f"{ctx.author.mention}: The following regions will no longer be recruited from: `{str(regionWhiteList)[1:-1]}`"
        )
